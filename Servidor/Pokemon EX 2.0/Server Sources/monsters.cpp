				...
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"immunity"))
				{
					if(readXMLString(tmpNode, "name", strValue))
					{
						...
						else if(tmpStrValue == "steel")
						{
							...
							mType->conditionImmunities |= CONDITION_STEEL;
						}
					...
					else if(readXMLString(tmpNode, "steel", strValue) && booleanString(strValue))
					{
						mType->damageImmunities |= COMBAT_STEELDAMAGE;
						mType->conditionImmunities |= CONDITION_STEEL;
					}
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"elements"))
		{
			...
			while(tmpNode)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"element"))
				{
					...
					else if(readXMLInteger(tmpNode, "steelPercent", intValue))
