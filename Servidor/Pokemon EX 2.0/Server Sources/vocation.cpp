	...
	for(xmlNodePtr configNode = p->children; configNode; configNode = configNode->next)
	{
		...
		else if(!xmlStrcmp(configNode->name, (const xmlChar*)"absorb"))
		{
			...
			if(readXMLInteger(configNode, "percentElements", intValue))
			{
				...
				voc->increaseAbsorb(COMBAT_GRASSDAMAGE, intValue);
      			voc->increaseAbsorb(COMBAT_ELECTRICDAMAGE, intValue);
      			voc->increaseAbsorb(COMBAT_FLYDAMAGE, intValue);
				voc->increaseAbsorb(COMBAT_ROCKDAMAGE, intValue);
				voc->increaseAbsorb(COMBAT_POISONDAMAGE, intValue);
                voc->increaseAbsorb(COMBAT_FIGHTDAMAGE, intValue);
				voc->increaseAbsorb(COMBAT_ROCKDAMAGE, intValue);
				voc->increaseAbsorb(COMBAT_GRASSDAMAGE, intValue);
				voc->increaseAbsorb(COMBAT_WATERDAMAGE, intValue);
				voc->increaseAbsorb(COMBAT_STEELDAMAGE, intValue);
				voc->increaseAbsorb(COMBAT_DRAGONDAMAGE, intValue);
			}
            ...
            if(readXMLInteger(configNode, "percentSteel", intValue))
				voc->increaseAbsorb(COMBAT_STEELDAMAGE, intValue);

            if(readXMLInteger(configNode, "percentWater", intValue))
				voc->increaseAbsorb(COMBAT_WATERDAMAGE, intValue);
				
            if(readXMLInteger(configNode, "percentElectric", intValue))
				voc->increaseAbsorb(COMBAT_ELECTRICDAMAGE, intValue);

			if(readXMLInteger(configNode, "percentEnergy", intValue))
				voc->increaseAbsorb(COMBAT_ENERGYDAMAGE, intValue);
				
			if(readXMLInteger(configNode, "percentFight", intValue))
				voc->increaseAbsorb(COMBAT_FIGHTDAMAGE, intValue);

            if(readXMLInteger(configNode, "percentRock", intValue))
				voc->increaseAbsorb(COMBAT_ROCKDAMAGE, intValue);
				
            if(readXMLInteger(configNode, "percentGhost", intValue))
				voc->increaseAbsorb(COMBAT_GHOSTDAMAGE, intValue);

            if(readXMLInteger(configNode, "percentGrass", intValue))
				voc->increaseAbsorb(COMBAT_GRASSDAMAGE, intValue);
            ...
            if(readXMLInteger(configNode, "percentFly", intValue))
				voc->increaseAbsorb(COMBAT_FLYDAMAGE, intValue);
			...
			if(readXMLInteger(configNode, "percentDragon", intValue))
				voc->increaseAbsorb(COMBAT_DRAGONDAMAGE, intValue);
		...
		else if(!xmlStrcmp(configNode->name, (const xmlChar*)"reflect"))
		{
			...
			if(readXMLInteger(configNode, "percentElements", intValue))
			{
				...
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_GRASSDAMAGE, intValue);
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_ELECTRICDAMAGE, intValue);
                voc->increaseReflect(REFLECT_PERCENT, COMBAT_WATERDAMAGE, intValue);
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_POISONDAMAGE, intValue);
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_DRAGONDAMAGE, intValue);
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_ROCKDAMAGE, intValue);
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_STEELDAMAGE, intValue);
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_GHOSTDAMAGE, intValue);
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_FIGHTDAMAGE, intValue);
				voc->increaseReflect(REFLECT_PERCENT, COMBAT_FLYDAMAGE, intValue);                                                                                                
			}
			...
			if(readXMLInteger(configNode, "chanceElements", intValue))
			{
				...
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_GRASSDAMAGE, intValue);
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_ELECTRICDAMAGE, intValue);
                voc->increaseReflect(REFLECT_CHANCE, COMBAT_WATERDAMAGE, intValue);
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_POISONDAMAGE, intValue);
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_DRAGONDAMAGE, intValue);
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_ROCKDAMAGE, intValue);
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_STEELDAMAGE, intValue);
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_GHOSTDAMAGE, intValue);
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_FIGHTDAMAGE, intValue);
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_FLYDAMAGE, intValue);
			}
			...
			if(readXMLInteger(configNode, "chanceWater", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_WATERDAMAGE, intValue);

			if(readXMLInteger(configNode, "chanceGrass", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_GRASSDAMAGE, intValue);
                
			if(readXMLInteger(configNode, "chanceRock", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_ROCKDAMAGE, intValue);
                
			if(readXMLInteger(configNode, "chanceSteel", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_STEELDAMAGE, intValue);
                
			if(readXMLInteger(configNode, "chanceFly", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_FLYDAMAGE, intValue);
				
			if(readXMLInteger(configNode, "chanceElectric", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_ELECTRICDAMAGE, intValue);				
                
			if(readXMLInteger(configNode, "chanceGhost", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_GHOSTDAMAGE, intValue);
				
			if(readXMLInteger(configNode, "chanceDragon", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_DRAGONDAMAGE, intValue);				
                
			if(readXMLInteger(configNode, "chanceFight", intValue))
				voc->increaseReflect(REFLECT_CHANCE, COMBAT_FIGHTDAMAGE, intValue);                                                                                                   	
